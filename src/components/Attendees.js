...
const ProgressBar = ({ numberOfAttendees }) => {
  return (
    <div className="iter-progress">
      <div
        className={`iter-progress-bar iter-${getProgressBarMode(numberOfAttendees)}`}
        style={{ width: `${numberOfAttendees * MAX_NUMBER_OF_ATTENDEES}%` }}
      >
...
const Attendees = ({ attendees, showRemoveAttendeeButtons }) => {
  ...
  const attendeeElements = attendees.map((attendee, index) => (
    <li key={index} className="attendees-list-item {{newItem date}}">
      ...
      <span className="timestamp">
        {formatDistance(attendee.timestamp, new Date(), { locale: nb, addSuffix: true })}
      </span>
      {showRemoveAttendeeButtons && (
        <button
          className="remove-attendee link-button"
          onClick={(e) => {
            e.preventDefault();
            removeRegisteredUser(attendee.name);
          }}
        >
          Fjern
        </button>
  ...
  return (
    <div>
      ...
      <ul className="attendees-list">{attendeeElements}</ul>
    </div>
  );
};
